...
public class WelcomePageActions extends WelcomePageRepo {
    ...
    public void verifyUserIsOnScreen(){
        if(ConfigFactory.getConfig().PlatformType().toString().toUpperCase().equals("mobile".toUpperCase())) {
            Assert.assertTrue("Element is not displayed",waitUntilElementDisplayed(welcomeSignUp_Text, 10));
            Assert.assertTrue("Element is not displayed",waitUntilElementDisplayed(welcomeLogin_Text, 10));
        }
        else{
    ...
    public void userClickOnSignupButton() {
        if(ConfigFactory.getConfig().PlatformType().toString().toUpperCase().equals("mobile".toUpperCase())) {
            click(signup_Button);
            click(signup_Button_WelcomeScreen);
        }
}}
